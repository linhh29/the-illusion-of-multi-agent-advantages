...
from bcp_prompt import get_init_archive, get_prompt, get_reflexion_prompt
from utils import random_id, load_questions, score_bcp, bootstrap_confidence_interval
...
class LLMAgentBase():
    ...
    def generate_prompt(self, input_infos, instruction) -> str:
        ...
        output_fields_and_description = {key: f"Your {key}." if not 'answer' in key else f"Your {key}. Return ONLY an string. DO NOT return anything other than the answer." for key in self.output_fields}
...
def evaluate_forward_fn(args, forward_str):
    ...
    task_queue = []
    for q in questions:
        task_content = f"Solve this deep search query:\n{q}\n"
    ...
    for q_idx, res in enumerate(results):
        try:
            ...
            correct = score_bcp(str(correct_answer), str(extracted_answer))
        except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_filename', type=str, default='bcp')
    ...
    parser.add_argument('--expr_name', type=str, default="bcp_gpt5")
    ...
    # echo python _bcp/search.py --data_filename dataset/bcp.jsonl --save_dir li_results/ --expr_name bcp_gpt-4o --n_repreat 1 --n_generation 10 --model ${AGENT_BASE_MODEL} --seed ${SEED} --max_new_tokens ${MAX_NEW_TOKENS} --valid_size 32
    ...
    # echo python _bcp/search.py --data_filename dataset/bcp.jsonl --save_dir li_results/ --expr_name bcp_gpt5 --n_repreat 1 --n_generation 10 --model ${AGENT_BASE_MODEL} --seed ${SEED} --max_new_tokens ${MAX_NEW_TOKENS} --valid_size 32 --gpt5_reasoning_effort ${GPT5_REASONING_EFFORT} --gpt5_verbosity ${GPT5_VERBOSITY}

    # /if eval_only/
    # export SEED=1/42
    # export SEARCH_EXPRNAME=bcp_gpt-5-medium-low_1019-225414
    # echo python _bcp/search.py --data_filename dataset/bcp.jsonl --save_dir li_results/ --n_repreat 1 --expr_name bcp_${AGENT_BASE_MODEL} --n_generation 10 --model ${AGENT_BASE_MODEL} --seed ${SEED} --max_new_tokens ${MAX_NEW_TOKENS} --gpt5_reasoning_effort ${GPT5_REASONING_EFFORT} --gpt5_verbosity ${GPT5_VERBOSITY} --eval_only --search_exprname ${SEARCH_EXPRNAME}
