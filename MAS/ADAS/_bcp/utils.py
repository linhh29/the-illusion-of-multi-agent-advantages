...
def score_bcp(target, prediction) -> bool: # TODO
    # Simple version, exact match    
    def are_equal_as(a, b):
        try:
            return str(a) == str(b)
        except (ValueError, TypeError):
            return False
    # check if punctuation and case insensitive
    if target.endswith('.'):
        target = target.rstrip('.')
    target = target.rstrip('.\t\n')
    # remove \\boxed{} and only keep what is inside
    # if prediction.startswith(r'\boxed{') and prediction.endswith("}"):
    #     prediction = prediction.removeprefix(r'\boxed{').removesuffix("}").rstrip(".")
    return are_equal_as(target.strip().lower(), prediction.strip().lower())
