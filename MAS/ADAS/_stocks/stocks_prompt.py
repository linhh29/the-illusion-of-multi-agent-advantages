...
LLM_debate = {
    ...
    debate_agents = [LLMAgentBase(['thinking', 'answer'], 'Debate Agent', temperature=0.8, role=role) for role in ["Financial Analyst", "Quant Researcher", "Risk Manager"]]
...
Take_a_step_back = {"thought": "Let LLM first think about the principles involved in solving this task which could be helpful. By understanding the underlying principles, the model can better reason through the problem and provide a more accurate solution.",
        ...
        principle_instruction = "What are the financial principles and concepts involved in solving this task? First think step by step. Then list all involved principles and explain them."
...
Role_Assignment = {"thought": "Similar to Auto-GPT and expert prompting, we can use dynamic control flow in the design to let the agent decide what expert we should use.",
        ...
        # Instruction for step-by-step reasoning
        cot_instruction = "Please think step by step and then solve the task."
        expert_agents = [LLMAgentBase(['thinking', 'answer'], 'Expert Agent', role=role) for role in ["Financial Analyst", "Quant Researcher", "Risk Manager"]]

        # Instruction for routing the task to the appropriate expert
        routing_instruction = "Given the task, please choose an Expert to answer the question. Choose from: Financial Analyst, Quant Researcher, Risk Manager."
        routing_agent = LLMAgentBase(['choice'], 'Routing agent')

        # Get the choice of expert to route the task
        choice = routing_agent([taskInfo], routing_instruction)[0]

        if 'financial analyst' in choice.content.lower():
            expert_id = 0
        elif 'quant researcher' in choice.content.lower():
            expert_id = 1
        elif 'risk manager' in choice.content.lower():
            expert_id = 2
        else:
...
base = """# Overview
You are an expert machine learning researcher testing various agentic systems. Your objective is to design building blocks such as prompts and control flows within these systems to solve complex tasks. Your aim is to design an optimal agent performing well on the Stocks dataset, a dataset featuring financial buying and selling activities.

## An example question from Stocks dataset:

**Question**: Here is some data on the stock prices of a few companies. Use this data to answer the following questions:

Alphabet Historical Stock Price Data
Date: 2025-12-29T05:00:00.000Z Open: 187.7100067139 High: 188.7599945068 Low: 185.9100036621 Close: 188.2200012207 Volume: 120006100 Dividends: 0.0 Stock Splits: 0.0
Date: 2025-12-30T05:00:00.000Z Open: 188.2400054932 High: 188.9900054932 Low: 186.9299926758 Close: 187.5399932861 Volume: 97687300 Dividends: 0.0 Stock Splits: 0.0
Date: 2025-12-31T05:00:00.000Z Open: 189.5700073242 High: 190.5599975586 Low: 186.4900054932 Close: 186.5 Volume: 120100500 Dividends: 0.0 Stock Splits: 0.0
Date: 2026-01-02T05:00:00.000Z Open: 189.8399963379 High: 192.9299926758 Low: 188.2599945068 Close: 188.8500061035 Volume: 148240500 Dividends: 0.0 Stock Splits: 0.0
Date: 2026-01-05T05:00:00.000Z Open: 191.7599945068 High: 193.6300048828 Low: 186.1499938965 Close: 188.1199951172 Volume: 183529700 Dividends: 0.0 Stock Splits: 0.0
[more data here...]

December 31, 2025:
Oliver bought 81 shares of Alphabet.

January 02, 2026:
Edward purchased 84 shares of Alphabet.

January 20, 2026:
Edward disposed 84 shares of Alphabet.

January 22, 2026:
Edward disposed 93 shares of Alphabet.

January 23, 2026:
Oliver sold 73 shares of Baidu.

January 26, 2026:
Oliver disposed 81 shares of Alphabet.

Each investor has completed several transactions and has already sold shares in one common stock but has not yet bought them. Based on when they would have had to buy these shares to achieve at least 0.1% overall portfolio profit when they sold, who has the first possible buy date (the first date in their list of valid dates) to reach this target, if all transactions were made at opening prices?


**Answer [Not Given]**: 
{
"answer": ['Edward', 'Oliver'],
"code": "def solve():\\n    # Your code here\\n    return {'investor_dates': {'Edward': ['December 29, 2025', 'December 30, 2025', 'December 31, 2025'], 'Oliver': ['December 29, 2025', 'December 30, 2025', 'December 31, 2025', 'January 02, 2026']}, 'comparison': {'Oliver': 'December 29, 2025', 'Edward': 'December 29, 2025'}, 'answer': ['Edward', 'Oliver']}"
}
...
def get_json_response_from_gpt(
    ...
    if model.startswith('gpt-3.5') or model.startswith('gpt-4'):
        response = client.chat.completions.create(
            ...
            temperature=temperature, seed=seed, max_tokens=max_new_tokens, stop=None, response_format={"type": "json_object"}
        )
        ...
        if 'errors' in response:
            return ValueError(f"IBM API Error: {response['errors'][0]['message']}")
        else:
            content = response['choices'][0]['message']['content']
...
class LLMAgentBase:
    ...
    def __init__(self, output_fields: list, agent_name: str, role='helpful assistant', 
                model=[AGENT_MODEL], 
                temperature=0.5,
                seed=[AGENT_SEED],
                max_new_tokens=[AGENT_MAX_NEW_TOKENS],
                gpt5_reasoning_effort=[AGENT_GPT5_REASONING_EFFORT],
                gpt5_verbosity=[AGENT_GPT5_VERBOSITY],
                ) -> None:
    ...
    def query(self, input_infos: list, instruction, iteration_idx=-1) -> list[Info]:
        ...
        response_json = get_json_response_from_gpt(prompt, self.model, system_prompt, self.temperature, self.seed, args.gpt5_reasoning_effort, args.gpt5_verbosity)
...
The full value is the accuracy on a validation question set (direct answer correctness). Your GOAL is to maximize the "full".
...
# Your task
You are deeply familiar with LLM prompting techniques and LLM agent works from the literature. Your goal is to maximize "full" accuracy by proposing interestingly new agents. 
...
def get_prompt(current_archive, model, seed, max_new_tokens,
               gpt5_reasoning_effort, gpt5_verbosity, adaptive=False):
    archive_str = ",\n".join([json.dumps(sol) for sol in current_archive])
    archive_str = f"[{archive_str}]"
    assert model in ['gpt-4o-2024-05-13', 'gpt-4o', 'gpt-5', 'openai/gpt-oss-120b', 'gemini-2.5-pro', 'gpt-3.5-turbo-0125', 'gpt-5-nano']
    prompt = base.replace("[AGENT_MODEL]", model) #li: Replaces the placeholder [AGENT_MODEL] with real model
    prompt = prompt.replace("[AGENT_SEED]", str(seed))
    prompt = prompt.replace("[AGENT_MAX_NEW_TOKENS]", str(max_new_tokens))
    prompt = prompt.replace("[AGENT_GPT5_REASONING_EFFORT]", gpt5_reasoning_effort)
    prompt = prompt.replace("[AGENT_GPT5_VERBOSITY]", gpt5_verbosity)
    prompt = prompt.replace("[ARCHIVE]", archive_str) #li: Replaces the placeholder [ARCHIVE] with real archive_str
    prompt = prompt.replace("[EXAMPLE]", json.dumps(EXAMPLE)) #li: Replaces the placeholder [EXAMPLE] with real EXAMPLE
    return system_prompt, prompt
