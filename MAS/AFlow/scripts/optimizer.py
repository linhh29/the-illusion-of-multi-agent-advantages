...
class Optimizer:
    ...
    def optimize(self, mode: OptimizerType = "Graph"):
        if mode == "Test":
            ...
            score = loop.run_until_complete(self.test(test_n))
            return None
    ...
    async def _optimize_graph(self):
        ...
        # print('111111111111111111', experience)
    ...
    def find_best_round(self, valid_path):
        round_best = []
        for round_number in range(1, 22):
            try:
                files = glob.glob(f'{valid_path}/round_{round_number}/*.csv')

                res = []
                for file in files:
                    tmp = file.split('/')[-1].split('_')[0]
                    res.append(float(tmp))
                round_best.append(sum(res)/len(res))
            except:
                round_best.append(-1)
        print(round_best)

        round_best_index = int(round_best.index(max(round_best))) + 1

        file_path = f"{valid_path}/round_{round_best_index}/graph.py"
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        # count the number of 'await' in the file
        count = content.count('await')

        return round_best_index, count


    async def test(self, test_n):
        # cot, cot-passK/cot-sc
        if self.dataset == 'SWE':
            rounds = [100, 102]
        else:
            rounds = [100, 101]  # You can choose the rounds you want to test here.
        # aflow (the best one on validation set)
        round_best_index, count = self.find_best_round(f"{self.root_path}/workflows")
        rounds.append(round_best_index)
        print(rounds, count)
        
        # Replace N = 3 with N = count in the best round's graph.py file

        graph_file_path = f"{self.root_path}/workflows/round_{rounds[1]}/graph.py"
        try:
            with open(graph_file_path, 'r', encoding='utf-8') as f:
                content = f.read()
            
            # Replace N = 3 with N = count (handling different spacing)
            # Match patterns like "N = 3", "N=3", "N =3", "N= 3"
            pattern = r'N\s*=\s*3'
            new_content = re.sub(pattern, f'N = {count}', content)
            
            with open(graph_file_path, 'w', encoding='utf-8') as f:
                f.write(new_content)
            
            logger.info(f"Updated N = 3 to N = {count} in {graph_file_path}")
        except Exception as e:
            logger.error(f"Failed to update graph.py file: {e}")
        ...
        for round in rounds:
            ...
            src_directory = directory.replace('workflows_test', 'workflows')
            for file in ['__init__.py', 'graph.py', 'prompt.py']:
                src_file = src_directory + '/' + file
                dst_file = directory + '/' + file
