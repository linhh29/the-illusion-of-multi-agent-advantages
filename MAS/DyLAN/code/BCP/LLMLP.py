...
from utils import parse_single_choice, most_frequent, extract_math_answer, is_equivalent
...
class LLMLP:
    ...
    def __init__(self, default_model_name, agents=4, agent_roles=[],
                 rounds=2, activation="listwise", qtype="single_choice", mtype="gpt-4o"):
        ...
        if qtype == "single_choice":
            self.cmp_res = is_equivalent
            self.ans_parser = extract_math_answer
        else:
            raise NotImplementedError(f"Error qtype: {qtype}. BCP only supports single_choice.")
