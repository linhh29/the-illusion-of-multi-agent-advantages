# -*- coding: utf-8 -*-
"""
STOCKS CoT-SC: multiple CoT samples per question + voting on extracted answer.
...
def parse_ensemble_override(arg_index):
    ...
    Placed after argv[7] CODE_EVAL_MODE for STOCKS.
    """
...
async def main():
    ...
    print(
        f"Processing {len(qa_pairs)} STOCKS questions with CoT-SC (ensemble={ensemble_size}, max {MAX_CONCURRENT} concurrent, code_eval_mode={CODE_EVAL_MODE})"
    )
    ...
    print(f"\n{'='*60}")
    print("FINAL EVALUATION RESULTS (STOCKS CoT-SC)")
    print(f"{'='*60}")
