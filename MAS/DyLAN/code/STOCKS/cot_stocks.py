# -*- coding: utf-8 -*-
"""
STOCKS CoT: single chain-of-thought response per question.
Evaluation same as llmlp_listwise_stocks (direct full match -> 1.0 else 0.0).
"""
import ast
import asyncio
import json
...
async def main():
    ...
    print(
        f"Processing {len(qa_pairs)} STOCKS questions with CoT (max {MAX_CONCURRENT} concurrent, code_eval_mode={CODE_EVAL_MODE})"
    )
    ...
    print(f"\n{'='*60}")
    print("FINAL EVALUATION RESULTS (STOCKS CoT)")
    print(f"{'='*60}")
