...
do
    echo "=========================================="
    echo "STOCKS CoT Run $run_num of 3"
    echo "=========================================="
    ...
    do
        ...
        python cot_stocks.py "$file" "$filename" "$MODEL" "$exp_name" "$ROLES" "$run_num" "$CODE_EVAL_MODE" > "$OUTPUT_DIR/${filename}_cot.log" 2>&1
    done
    echo "Run $run_num completed!"
done
echo "All STOCKS CoT runs completed!"
