...
from utils import most_frequent, extract_patch
...
class LLMLP:
    ...
    def __init__(self, default_model_name, agents=4, agent_roles=[],
                 rounds=2, activation="listwise", qtype="code_patch", mtype="gpt-4o"):
        ...
        self.qtype = qtype
        if qtype == "code_patch":
            # For patches, we compare by string similarity (exact match or high similarity)
            self.cmp_res = lambda x, y: x == y if x and y else False
            self.ans_parser = extract_patch
