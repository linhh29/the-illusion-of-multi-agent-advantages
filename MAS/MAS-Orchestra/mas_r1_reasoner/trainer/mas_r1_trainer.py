...
from mas_r1_reasoner.trainer.utils.mas_benchmark_export import export_mas_validation_samples
...
class MASR1Trainer(RayPPOTrainer):
    ...
    def init_workers(self):
        """Skip Ray vLLM workers when orchestrator uses an external OpenAI-compatible server."""
        import os
        from types import SimpleNamespace

        if os.environ.get("MAS_ORCHESTRATOR_OPENAI_BASE", "").strip():
            self.async_rollout_mode = False
            self.actor_rollout_wg = SimpleNamespace(world_size=1)
            main_rank_print(
                "MAS_ORCHESTRATOR_OPENAI_BASE is set: skipping in-process vLLM/Ray rollout; "
                "orchestrator calls the remote OpenAI-compatible API (see rollout_generation)."
            )
            return
        super().init_workers()
    ...
    def _validate(self):
        ...
        for batch_idx, batch_dict in enumerate(self.val_dataloader):
            ...
            mas_export_dir = self.config.trainer.get("mas_export_dir")
            if mas_export_dir:
                export_mas_validation_samples(
                    export_dir=str(mas_export_dir),
