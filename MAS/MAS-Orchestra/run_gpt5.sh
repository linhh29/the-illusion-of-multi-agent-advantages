...
export MAS_API_MAX_CONCURRENCY="${MAS_API_MAX_CONCURRENCY:-100}"
# Same baseline seconds as run_gpt4o.sh; sampler applies 3× for gpt-5 (effective default 1800s unless overridden by env).
...
# Sub-agent uses ChatCompletionSampler; gpt-5 requests always use temperature=1.0 (see sampler).
#  SWE-Bench-Lite BrowseComp+
AGENT_MODEL=gpt-5
for RUN_SUFFIX in run1 run2 run3; do
  for dataset in GPQA HLEMATH STOCKS BrowseComp+; do
