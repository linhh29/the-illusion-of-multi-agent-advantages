...
except ImportError:  # optional (tree_sitter); BCP grading uses plain __call__ only
    class FormatError(Exception):
        pass

    class BaseFormatter:  # noqa: PYI024
        pass
...
class LLMsConfig:
    ...
    def default(cls):
        ...
        if cls._default_config is None:
            # Prefer vendored config next to this package (works without ``chdir`` to vendor root).
            _pkg_root = Path(__file__).resolve().parent.parent
            config_paths = [
                _pkg_root / "config" / "config2.yaml",
...
class AsyncLLM:
    def __init__(self, config, system_msg:str = None):
        ...
        self.config = config
        api_key = self.config.key or os.environ.get("OPENAI_API_KEY")
        client_kw: Dict[str, Any] = {"api_key": api_key}
        bu = getattr(self.config, "base_url", None) or ""
        if bu and str(bu).strip():
            client_kw["base_url"] = str(bu).strip()
        self.aclient = AsyncOpenAI(**client_kw)
        self.sys_msg = system_msg
        self.usage_tracker = TokenUsageTracker()
        
    async def __call__(self, prompt):
        """
        Default plain-text LLM call, used by all tasks that do not
    ...
    async def call_reverse_answer_code(self, prompt: str) -> str:
        """
        Structured call for STOCKS tasks: ask the model to return
        JSON with fields analysis / answer / code using JSON schema.
        Other datasets should keep using the default __call__.
        """
