...
<thought> entry: Revise your previous proposal or propose a new architecture if necessary, using the same format as the example response. 
...
<code> entry: Update the code entry based on your reflection and thought. 

For those sub-tasks that have already been resolved, please use them to simplify the original question, and do not involve them again in the revised new sub-tasks. You can access the outputs from some agent in the previous round by call `extra_info['round_{last round id}_{agent name}'][{output_field}]`. The corresponding saving logic is as below:

```python
... code in class LLMAgentBase

extra_info[f"round_{extra_info['n']}_{self.__repr__()}"] = response_json
```
You can append these outputs to the sub-agent instruction to update the question information.
