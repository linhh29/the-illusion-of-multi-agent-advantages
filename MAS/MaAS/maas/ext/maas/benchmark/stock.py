...
# Verbatim copy of AFlow benchmarks/stocks.py `Instruction` (lines 12–37). Keep in sync with
# /data/qin/lhh/MAS-Eval-Foundation/AFlow/benchmarks/stocks.py — do not edit unless upstream changes.
...
# AFlow ``backup_workspace/.../STOCKS/workflows/template/op_prompt.py`` — ``SC_ENSEMBLE_PROMPT``
# (used with ``call_reverse_answer_code`` / ``acall_reverse_answer_code``).
STOCKS_AFLOW_SC_ENSEMBLE_PROMPT = """
...
STOCKS_AFLOW_ANSWER_GENERATION_PROMPT = """
...
def extract_stock_reference_answer(problem: Dict[str, Any]) -> List[str]:
    """
    Same as AFlow ``benchmarks/stocks.StocksBenchmark._extract_reference_answer``.
    """
...
def parse_stock_model_output(raw_output: Any) -> Tuple[str, str, str]:
    """
    Same as AFlow ``benchmarks/stocks.StocksBenchmark._parse_model_output`` (verbatim logic).
    """
    raw_str = None
    answer = ""
    code = ""
...
def stocks_evaluate_direct_answer(
    model_answer: Any, reference_answer: List[str]
) -> Tuple[bool, int]:
    """
    Same as AFlow ``benchmarks/stocks.StocksBenchmark._evaluate_direct_answer``:
...
class StockBenchmark(BaseBenchmark):
    """
    Benchmark for stock trading analysis tasks.

    Evaluation follows the same logic as AFlow's StocksBenchmark:
    ...
    def __init__(
        ...
        logger.info(f"StockBenchmark initialised with eval_mode='{self.eval_mode}'")
    ...
    def _evaluate_direct_answer(
        ...
        return stocks_evaluate_direct_answer(model_answer, reference_answer)
    ...
    async def evaluate_problem(self, problem: dict, graph: Callable):
        ...
        input_text = problem.get("problem", "") + STOCKS_AFLOW_INSTRUCTION
        expected_names: List[str] = extract_stock_reference_answer(problem)
        ...
        try:
            ...
            raw_output_str, model_answer, code_str = parse_stock_model_output(output)
            ...
            if score == 0.0:
                self.log_mismatch(
                    ...
                    extract_answer_code="stocks_benchmark",
                )
            ...
            logger.info(
                f"Maximum retries reached for a stocks sample. "
