...
class OpenAILLM(BaseLLM):
    ...
    def _cons_kwargs(self, messages: list[dict], timeout=USE_CONFIG_TIMEOUT, **extra_kwargs) -> dict:
        ...
        elif "chatgpt-4o-latest" in self.model:
            # Some newer models use max_completion_tokens
            token_key = "max_completion_tokens"
        else:
        ...
        # Special handling for o1-series and GPT-5
        if "o1-" in self.model:
            # o1-series: temperature must be 1, no max_tokens
            kwargs["temperature"] = 1
            kwargs.pop(token_key, None)
        elif "gpt-5" in self.model:
            # GPT-5: temperature must be 1 (default value)
            kwargs["temperature"] = 1
    ...
    async def acall_reverse_answer_code(
        self,
        msg: str,
        ...
        """
        AFlow-compatible STOCKS call: JSON with ``analysis`` / ``answer`` / ``code`` via
        OpenAI Structured Outputs (``response_format`` ``json_schema`` with ``strict: true``).
        No fallback — the caller must use an API endpoint and model that support this mode.
        """
        ...
        kwargs = self._cons_kwargs(message, timeout=self.get_timeout(timeout))
    ...
    async def acompletion_text(self, messages: list[dict], stream=False, timeout=USE_CONFIG_TIMEOUT) -> str:
        ...
        result_text = self.get_choice_text(rsp)
        
        # Debug: Log if response is empty for GPT-5
        if not result_text and "gpt-5" in self.model:
            logger.warning(f"GPT-5 returned empty response!")
            logger.warning(f"Response object: {rsp}")
            logger.warning(f"Choices: {rsp.choices if rsp.choices else 'NO CHOICES'}")
            if rsp.choices:
                choice = rsp.choices[0]
                logger.warning(f"Message content: '{choice.message.content}'")
                logger.warning(f"Finish reason: {choice.finish_reason}")
                if hasattr(choice.message, 'refusal'):
                    logger.warning(f"Refusal: {choice.message.refusal}")
    ...
    def _get_max_tokens(self, messages: list[dict]):
        # GPT-5 is a reasoning model and needs much more tokens for reasoning + output
        # This must be checked FIRST before auto_max_tokens check
        if "gpt-5" in self.model:
            # Allow up to 32684 tokens for reasoning models (reasoning tokens + output tokens)
            # Don't use get_max_completion_tokens because gpt-5 is not in TOKEN_MAX dict
            return 32684
