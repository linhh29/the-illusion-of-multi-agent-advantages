#!/bin/bash
# Stock CoT evaluation (AFlow StocksBenchmark-aligned). Levels 2–6, three runs.
# Up to 50 concurrent async STOCKS tasks per run (asyncio.Semaphore in stock_standalone_eval).
#
# Smoke test (one problem per level):   STOCK_LIMIT=1 bash workspace/run_stock_cot_3x.sh
...
for i in 1 2 3; do
    ...
    python -m workspace.standalone_cot_eval \
        ...
        --max_concurrent 30 \
        --max_tokens 32768 \
        --output_path "workspace/results_cot_stock_${MODEL}_run${i}.json"
    run_ec=$?
