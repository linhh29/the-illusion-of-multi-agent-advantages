...
from smfr_prompt import get_init_archive, get_prompt, get_reflexion_prompt
from utils import random_id, load_smfr_examples, score_smfr, to_dict
...
def evaluate_forward_fn(args, forward_str, mode, solution_name=None):
    ...
    if SEARCHING_MODE:
        val_questions = load_smfr_examples(split='validate') * args.n_repreat
    else:
        val_questions = load_smfr_examples(split='test') * args.n_repreat
    ...
    # for smfr, write down results in a prediction file, .json file
    pred_file = Path(__file__).resolve().parent.parent/"li_results"/"smfr"/EXPR_NAME/f"{mode}_{solution_name}_predictions.json"
    ...
    # Evaluate results
    if SEARCHING_MODE:
        ref_file = Path(__file__).resolve().parent.parent/"dataset"/"smfr_validate.jsonl"
        full, partial_match, cfull, cpartial_match, code_failures, total, res_per_depth = score_smfr(pred_file, ref_file, mode="validate")
    else:
        ref_file = Path(__file__).resolve().parent.parent/"dataset"/"smfr_test.jsonl"
        full, partial_match, cfull, cpartial_match, code_failures, total, res_per_depth = score_smfr(pred_file, ref_file, mode="test")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--expr_name', type=str, default="smfr_gpt5")
    ...
    # export SEARCH_EXPRNAME=smfr_gpt-4o_0407-074914
    # echo python _smfr/search.py --save_dir li_results/ --expr_name smfr_gpt-4o --n_repreat 1 --n_generation 15 --model ${AGENT_BASE_MODEL} --seed ${SEED} --max_new_tokens ${MAX_NEW_TOKENS} --eval_only --search_exprname ${SEARCH_EXPRNAME}
    ...
    # echo python _smfr/search.py --save_dir li_results/ --expr_name smfr_gpt5 --n_repreat 1 --n_generation 10 --model ${AGENT_BASE_MODEL} --seed ${SEED} --max_new_tokens ${MAX_NEW_TOKENS} --gpt5_reasoning_effort ${GPT5_REASONING_EFFORT} --gpt5_verbosity ${GPT5_VERBOSITY}

    # /eval_only/
    # export SEED=1/42
    # export SEARCH_EXPRNAME=smfr_gpt5-medium-low_1127-181117
    # export SEARCH_EXPRNAME=smfr_gpt5-medium-low_1128-024003
    # export SEARCH_EXPRNAME=smfr_gpt5-medium-low_1128-075436
    # echo python _smfr/search.py --save_dir li_results/ --expr_name smfr_gpt5 --n_repreat 1 --n_generation 5 --model ${AGENT_BASE_MODEL} --seed ${SEED} --max_new_tokens ${MAX_NEW_TOKENS} --gpt5_reasoning_effort ${GPT5_REASONING_EFFORT} --gpt5_verbosity ${GPT5_VERBOSITY} --eval_only --search_exprname ${SEARCH_EXPRNAME}
