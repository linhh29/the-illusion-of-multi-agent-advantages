...
import evaluate_smfr_answer_code as eval_smfr
...
def score_smfr(pred_file, ref_file, mode="test") -> bool: # TODO  
    timeout = 30 # seconds
    ...
    references = eval_smfr.load_reference_data(reference)

    print(f"Loading model outputs from: {model_output}")
    model_outputs = eval_smfr.load_model_outputs(model_output)
    ...
    executor = eval_smfr.SafeCodeExecutor(timeout=timeout)
    ...
    for idx, model_output in enumerate(model_outputs):
        ...
        try:
            ...
            answer = []
        is_full, partial_count = eval_smfr.evaluate_direct_answer(
            #model_output['answer'],
        ...
        try:
            ...
            code = ""
        code_full, code_partial, code_failed = eval_smfr.evaluate_code_output(
            #model_output['code'],
            code,
            reference_answer,
            executor
...
def load_smfr_examples(split: str):
    # Load SMFR examples from the corresponding jsonl file in AFlow/data/datasets/smfr_{split}.jsonl
    dataset_path = Path(__file__).resolve().parent.parent.parent/"AFlow"/"data"/"datasets"/"smfr_{}.jsonl".format(split)
...
if __name__ == "__main__":
    # Example usage
    # load_smfr_examples(split='validate')
    # load_smfr_examples(split='test')
