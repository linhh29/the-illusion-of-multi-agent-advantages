...
async def process_single_question(idx, que, ans, model, roles, activation, qtype, dir_name, exp_name):
    ...
    try:
        ...
        # Evaluate the answer
        # acc = evaluate_answer(res, ans)
        acc = grade_bcp_answer(que, res, ans)
        ...
        pred_str = str(res) if res else "None"
...
async def main():
    ...
    print(f"Processing {len(qa_pairs)} questions with async (max {MAX_CONCURRENT} concurrent)")
    ...
    async def process_with_semaphore(idx, que, ans):
        async with semaphore:
            result = await process_single_question(idx, que, ans, MODEL, ROLES, ACTIVATION, TYPE, DIR_NAME, EXP_NAME)
