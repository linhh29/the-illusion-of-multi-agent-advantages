import random
import re
from utils import extract_math_answer, generate_answer
from prompt_lib import ROLE_MAP_MATH, construct_ranking_message, construct_message, SYSTEM_PROMPT_HLEMATH


class LLMNeuron:
    
    def __init__(self, role, mtype="gpt-4o", ans_parser=extract_math_answer, qtype="math_exp"):
        self.role = role
        self.mtype = mtype
        self.qtype = qtype
        ...
        if mtype in ["gpt-4o", "gpt-5"]:
            self.model = mtype
        else:
            raise NotImplementedError(f"Error init model type: {mtype}. Supported models: gpt-4o, gpt-5")
...
def listwise_ranker_2(responses, question, qtype, model="gpt-4o"):
    if model not in ["gpt-4o", "gpt-5"]:
        raise NotImplementedError(f"Error init model type: {model}. Supported models: gpt-4o, gpt-5")
