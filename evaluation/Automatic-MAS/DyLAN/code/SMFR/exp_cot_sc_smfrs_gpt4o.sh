...
do
    echo "=========================================="
    echo "SMFR CoT-SC Run $run_num of 3"
    echo "=========================================="
    ...
    do
        ...
        python cot_sc_smfr.py "$file" "$filename" "$MODEL" "$exp_name" "$ROLES" "$run_num" "$CODE_EVAL_MODE" "$number" > "$OUTPUT_DIR/${filename}_cot_sc.log" 2>&1
    done
    echo "Run $run_num completed!"
done
echo "All SMFR CoT-SC runs completed!"
