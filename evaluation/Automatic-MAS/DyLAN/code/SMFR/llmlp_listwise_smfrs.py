# -*- coding: utf-8 -*-
"""
SMFR evaluation in DyLAN. Aligned with AFlow benchmarks/smfr.py:
- Same data format: smfr_validate.jsonl / smfr_test.jsonl (problem, answer)
...
async def main():
    ...
    print(f"Processing {len(qa_pairs)} SMFR questions (max {MAX_CONCURRENT} concurrent, code_eval_mode={CODE_EVAL_MODE})")
    ...
    # Aggregate smfr-specific metrics (direct/code full/partial/execution failure)
    ...
    print(f"\n{'='*60}")
    print("FINAL EVALUATION RESULTS (SMFR, same metric as AFlow)")
    print(f"{'='*60}")
