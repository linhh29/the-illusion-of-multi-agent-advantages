...
# Must match ``AFlow/benchmarks/smfr.py`` ``Instruction`` (appended to ``problem`` in ``SmfrsBenchmark.evaluate_problem``).
SMFR_INSTRUCTION = """
...
def _smfr_reference_names(row: Dict[str, Any]) -> List[str]:
    """Same reference extraction as ``benchmarks.smfr.SmfrsBenchmark._extract_reference_answer``."""
...
DATASETS: Dict[str, DatasetSpec] = {
    ...
    "SMFR": DatasetSpec(
        key="SMFR",
        result_dir_name="SMFR",
        jsonl_name="smfr_test.jsonl",
        orchestrator_hf=_HARMONY_LOW,
        problem_type="harmony_minimal",
        init_archive=["COT", "COT_SC", "Reflexion", "LLM_debate"],
        no_decompose=False,
...
def row_to_parquet_record(
    ...
    if dataset_spec.key == "SWE":
        ...
        gt = row.get("patch", "")
    elif dataset_spec.key == "SMFR":
        question = row.get("problem", "") + SMFR_INSTRUCTION
        gt = json.dumps(_smfr_reference_names(row), ensure_ascii=False)
    else:
