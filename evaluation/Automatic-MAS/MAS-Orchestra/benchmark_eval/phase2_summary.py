...
SMFR: DyLAN-compatible metrics + per-``source_split`` (depth) cost and token totals.
...
def write_phase2_dataset_summary(
    *,
    ...
    SMFR appends DyLAN-style extended metrics and per-depth (``source_split``) cost.
    """
    n = len(rows)
    ...
    # SMFR aggregates (filled when spec.key == SMFR)
    smfr_docs: List[Tuple[int, Dict[str, Any], Dict[str, Any], Optional[Dict[str, Any]]]] = []
    ...
    for idx, raw_row in enumerate(rows):
        ...
        if doc is None:
            ...
            if spec.key == "SMFR":
                smfr_docs.append((idx, raw_row, None, None))
            continue
        completed_cnt += 1
        ca = doc.get("accuracy")
        if ca is not None:
        ...
        if spec.key == "SMFR":
            sm = doc.get("smfr_eval") if isinstance(doc.get("smfr_eval"), dict) else None
            smfr_docs.append((idx, raw_row, doc, sm))
    ...
    if spec.key == "SMFR":
        lines.extend(_smfr_dylan_style_extension(rows, smfr_docs))
...
def _smfr_dylan_style_extension(
    rows: List[Dict[str, Any]],
    smfr_docs: List[Tuple[int, Dict[str, Any], Dict[str, Any], Optional[Dict[str, Any]]]],
) -> List[str]:
    """DyLAN ``smfr_test_*.txt``-style tail: booleans line, token lines, detailed match counts, per-depth + cost."""
    flags: List[bool] = []
    for idx, raw_row, doc, sm in smfr_docs:
    ...
    out: List[str] = [
        "",
        "============================================================",
        "SMFR extended (DyLAN / AFlow-aligned)",
        "============================================================",
