...
GPQA / HLEMATH / SWE-Bench / BrowseComp+ / SMFR →
...
from .datasets_aflow import _smfr_reference_names
...
def score_smfr(prediction: str, reference_names: List[str]) -> Tuple[float, str]:
    """
    Aligns with ``AFlow/benchmarks/smfr.py`` ``evaluate_problem`` aggregate score:
    1.0 if direct-answer evaluation is a full match, else 0.0.
    Uses ``SmfrsBenchmark._parse_model_output`` and ``_evaluate_direct_answer``.
    """
    ensure_aflow_on_syspath()
    from benchmarks.smfr import SmfrsBenchmark

    b = SmfrsBenchmark("SMFR", "", "")
...
def smfr_eval_breakdown(prediction: str, raw_row: Dict[str, Any]) -> Dict[str, Any]:
    """
    Per-sample SMFR metrics aligned with DyLAN ``cot_smfrs.py`` / AFlow ``SmfrsBenchmark``:
    direct_full, direct_partial_count, code_full, code_partial, code_failed, score (direct-full only).
    """
    ensure_aflow_on_syspath()
    from benchmarks.smfr import SmfrsBenchmark

    ref = _smfr_reference_names(raw_row)
    try:
        b = SmfrsBenchmark("SMFR", "", "")
...
# GPQA, HLEMATH, SWE-Bench, BrowseComp+, SMFR → harmony_parser/minimal.py (DoM Low).
_HARMONY_PROBLEM_TYPE_BY_DATASET_KEY: Dict[str, str] = {
    "GPQA": "harmony_minimal",
    "HLEMATH": "harmony_minimal",
    "SWE": "harmony_minimal",
    "BCP": "harmony_minimal",
    "SMFR": "harmony_minimal",
}
