"""
SMFR: second-pass structured completion aligned with AFlow ``call_reverse_answer_code``.
...
``analysis``, ``answer`` (string), and ``code`` for ``SmfrsBenchmark._parse_model_output``.

Disable with env ``MAS_SMFR_STRUCTURED_OUTPUT=0`` (or ``false`` / ``no``).
"""
...
def _structured_output_disabled() -> bool:
    raw = (os.environ.get("MAS_SMFR_STRUCTURED_OUTPUT") or "1").strip().lower()
...
def maybe_augment_smfr_prediction(
    pred_text: str,
    *,
    question: str,
    agent_model: str,
    ...
    try:
        ...
        print(
            f"benchmark_eval.smfr_structured_output: keeping MAS draft ({e})",
            file=sys.stderr,
            flush=True,
        )
    return pred_text
