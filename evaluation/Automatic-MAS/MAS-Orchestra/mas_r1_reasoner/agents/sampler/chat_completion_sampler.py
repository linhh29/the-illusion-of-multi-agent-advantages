...
def _chat_http_timeout_sec(model: str | None = None) -> float:
    """Per-request read timeout for chat.completions (env ``MAS_OPENAI_CHAT_TIMEOUT_SEC``, default 120)."""
    try:
        t = float(os.environ.get("MAS_OPENAI_CHAT_TIMEOUT_SEC", "120"))
    except ValueError:
        t = 120.0
...
class ChatCompletionSampler(SamplerBase):
    ...
    def __init__(
        ...
        self.client = AsyncOpenAI(**build_openai_client_kwargs())
        _d = get_sampler_defaults("ChatCompletionSampler")
        ...
        _mt = self._convert_to_basic_type(max_tokens if max_tokens is not None else _d.get("max_tokens"))
        self.max_tokens = int(_mt) if _mt is not None else None
        mct = self._convert_to_basic_type(
            max_completion_tokens if max_completion_tokens is not None else _d.get("max_completion_tokens")
        )
        self.max_completion_tokens = int(mct) if mct is not None else None
        ...
        while True:
            ...
            try:
                ...
                if self.mock_output:
                    ...
                    _htt = _chat_http_timeout_sec(self.model)
                    ...
                    if usage is not None:
                        ...
                        record_completion(str(safe_model), int(pt), int(ct), provider="openai")
                ...
                if not self.mock_output:
                    ...
                    if not str(content).strip():
                        print(
                            f"\n✗ Empty completion after HTTP 200 (Trial {trial + 1}) — "
                            f"retrying (often context too long or transient API issue)"
                        )
                ...
                print(f"  - Temperature: {temperature if temperature is not None else self.temperature}")
                print(f"  - Hint: raise MAS_OPENAI_CHAT_TIMEOUT_SEC (e.g. 600) for long BrowseComp+ prompts")
                ...
                print(f"  - Temperature: {temperature if temperature is not None else self.temperature}")
                exception_backoff = min(2**trial, 60.0)
                print(f"  - Backoff: {exception_backoff}s then retry (transient 4xx/overload)")
                await asyncio.sleep(exception_backoff)
                trial += 1
                if trial >= 5:
