...
class SWEBenchmark(BaseBenchmark):
    ...
    async def evaluate_problem(self, data: dict, graph: Callable) -> Tuple[str, str, str, float, float]:
        ...
        max_retries = 10
        max_time_limit = 600  # 1 hour in seconds
        ...
        for attempt in range(max_retries):
            ...
            if elapsed_time > max_time_limit:
                logger.info(f"Evaluation exceeded time limit of {max_time_limit} seconds. Returning error.")
                return input_text, f"Evaluation timeout: exceeded {max_time_limit} seconds", expected_output, 0.0, 0.0
            # Portable stale-log cleanup (upstream AFlow used a machine-specific absolute path).
            log_dir = os.path.join(str(judge_path), "run_evaluation", instance_id.replace("-", "_") + "__")
            print(log_dir)
            if os.path.exists(log_dir):
                try:
                    ...
                    logger.warning(f"Failed to delete log directory {log_dir}: {del_e}")
            try:
                ...
                logger.info(f"Timeout error on attempt {attempt + 1}/{max_retries}. Retrying...")
                ...
                logger.info(f"Error on attempt {attempt + 1}/{max_retries}. Error: {e}. Retrying...")
                if attempt == max_retries - 1:
                    logger.info("Maximum retries reached. Skipping this sample.")
                    return input_text, str(e), expected_output, 0.0, 0.0
                continue
        
        # 如果循环结束仍未成功且没有异常（所有尝试都失败但都正常执行），直接返回错误结果
        logger.info("All retries failed. Skipping this sample.")
        return input_text, "All retries failed", expected_output, 0.0, 0.0
