...
# AFlow/benchmarks/swe_utils.py uses ``asyncio.Semaphore(20)``. MAS-Orchestra scoring invokes
# ``asyncio.run(run_swebench_evaluation)`` from thread-pool workers (fresh event loop per call);
# a module-level asyncio.Semaphore binds to one loop. Keep the same slot count (20) with threading.
subprocess_semaphore = threading.Semaphore(20)
...
async def run_swebench_evaluation(judge_path: str, instance_id: str, extracted_answer: str, technique: str, solution_name, code_snippet=None, file_path=None):
    ...
    # for i in range(1, 6):
    #     if os.path.exists(judge_path + f'/results/{i}/' + f'_{instance_id}_{solution_name}.json'):
    #         continue
    #     else:
    #         current_number = i
    #         break
    ...
    path_to_prediction = judge_path + '/results/' + f'_{instance_id}_{solution_name}.json'
    ...
    # read the report (verbatim AFlow/benchmarks/swe_utils.py)
    prefix = instance_id.replace('-', '_')
    result_path = judge_path + '/reports/' + f'gpt-4o.{prefix}__.json'
    # swebench.harness.reporting.make_run_report also writes ``{model}.{run_id}.json`` under report_path
    model_slug = str(prediction["model_name_or_path"]).replace("/", "__")
    result_path_harness = judge_path + '/reports/' + f'{model_slug}.{run_id}.json'
    ...
    cmd = [
        ...
        "--run_id", str(run_id),
        "--report_path", judge_path + '/reports'
        # "--namespace", ''
    ]
    ...
    await loop.run_in_executor(None, subprocess_semaphore.acquire)
    try:
        while True:
            try:
                # 创建异步子进程
                ...
                if process.returncode == 0:
                    ...
                    print(f"Error running evaluation: {stderr.decode()}")
    ...
    total_tests = 0
    passed_tests = 0
    report_file = result_path if os.path.exists(result_path) else None
    if report_file is None and os.path.exists(result_path_harness):
        report_file = result_path_harness
    if report_file is not None:
        ...
        print(f"Report file not found at {result_path} or {result_path_harness}")
        score = 0.0
        percentage = 0.0
