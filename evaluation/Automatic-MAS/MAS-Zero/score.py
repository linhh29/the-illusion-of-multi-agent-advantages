...
class DataScorer:
    ...
    def _is_smfr_dataset(self):
        if not self.dataset:
            return False
        name = self.dataset.lower().strip()
        return name == "workflow_search/smfr" or "smfr_synthetic" in name

    def _load_smfr_executor(self):
        eval_dir = Path(__file__).resolve().parent / "smfr_synthetic_dataset" / "evaluate"
        if eval_dir.exists():
    ...
    def _extract_smfr_answer_blob(self, response_text: str) -> str:
    ...
    def _parse_smfr_model_output(self, response_text: str) -> dict:
        answer_blob = self._extract_smfr_answer_blob(response_text)
    ...
    def _evaluate_smfr_candidate(self, correct_answer, candidate):
        ...
        executor = self._load_smfr_executor()
    ...
    async def run_score(self, answer, extracted_answer, use_oracle_verifier, judge_path, instance_id, n, code_snippet):
        ...
        if self._is_swe_dataset():
            ...
            return 0.0
        elif self._is_smfr_dataset():
            try:
                return float(self._evaluate_smfr_candidate(answer, extracted_answer))
            except Exception:
                return 0.0
    ...
    async def score(self, example_id, n, prompt_message, question, response_text, answer, sub_tasks_text, use_oracle_verifier, judge_path, response_path,
        ...
        if self._is_swe_dataset():
            ...
            extracted_answer = self._parse_smfr_model_output(response_text)
        else:
            try:
