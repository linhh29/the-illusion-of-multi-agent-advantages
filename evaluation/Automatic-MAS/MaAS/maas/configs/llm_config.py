...
class LLMConfig(YamlModel):
    ...
    project_id: Optional[str] = None  # For IBM Cloud and similar providers
    ...
    # For Chat Completion: default 32k; gpt-4o (non-mini) uses 16k via model_validator below.
    max_token: int = 32768
    ...
    def _default_max_token_gpt4o_16k_others_32k(self):
        """gpt-4o / versioned gpt-4o-* → 16k; all other models keep 32k (32768).

        When max_token still equals the class default 32768 and the model is gpt-4o
        (excluding gpt-4o-mini), set 16384. Explicit overrides (any other value) are kept.
        To force 32k on gpt-4o, set max_token to a value other than 32768 (e.g. 32769).
        """
        ml = (self.model or "").lower()
        if "gpt-4o-mini" in ml:
            return self
        if "gpt-4o" in ml and self.max_token == 32768:
