...
class BaseBenchmark(ABC):
    ...
    def save_results_to_csv(self, results: List[Tuple[Any, ...]], columns: List[str]):
        ...
        # Calculate cost statistics if cost column exists
        if "cost" in df.columns:
            total_cost = df["cost"].max()  # Total accumulated cost
            avg_cost = total_cost / len(df) if len(df) > 0 else 0.0
            # Drop cost before saving to CSV
            df_to_save = df.drop(columns=["cost"])
        else:
            total_cost = 0.0
            avg_cost = 0.0
            df_to_save = df
        
        # Estimate token usage (rough estimation based on cost)
        # Assuming ~$0.01 per 1000 tokens for common models
        token = int(total_cost * 100000) if total_cost > 0 else 0
        ...
        return avg_score, avg_cost, total_cost, token
    ...
    async def run_evaluation(self, graph: Callable, va_list: List[int], is_test: bool, sample: int, is_textgrad: bool = False, max_concurrent_tasks: int = 30):
        ...
        if is_test == True:
            ...
            average_score, avg_cost, total_cost, token = self.save_results_to_csv(results, columns)
            logger.info(f"Average score on {self.name} dataset: {average_score:.5f}")
            logger.info(f"Total cost: ${total_cost:.5f}, Avg cost: ${avg_cost:.5f}, Tokens: {token}")
                
            return average_score, avg_cost, total_cost, token
