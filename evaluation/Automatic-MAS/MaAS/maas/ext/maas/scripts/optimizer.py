...
def _smfr_aggregate_from_csv(level_dir: str) -> dict:
...
class Optimizer:
    ...
    async def test(self):
        if self.dataset == "Smfr":
            return await self._test_smfr_multi_level()
        return await self._test_single()

    async def _test_single(self):
        """Standard single-dataset test (used by all non-Smfr benchmarks)."""
        data = []
    ...
    async def _test_smfr_multi_level(self):
        """Test Smfr benchmark across complexity levels 2-6, recording each result."""
        graph_path = f"{self.root_path}/test"
        ...
        # --- iterate over levels ---
        smfr_levels = self.smfr_levels if hasattr(self, "smfr_levels") else [2, 3, 4, 5, 6]
        all_level_data = []
        summary = {}

        for level in smfr_levels:
            data_file = f"maas/ext/maas/data/smfr_level_{level}.jsonl"
            ...
            logger.info(f"{'='*60}")
            logger.info(f"Evaluating Smfr level {level}  ({data_file})")
            logger.info(f"{'='*60}")
            ...
            # --- attach smfr direct/code aggregate from eval_details ---
            try:
                level_result["smfr_aggregate"] = _smfr_aggregate_from_csv(level_dir)
            except Exception as e:
                logger.warning(f"Could not compute smfr_aggregate for level {level}: {e}")
        ...
        logger.info("")
        logger.info("=" * 60)
        logger.info("  SMFR MULTI-LEVEL TEST SUMMARY")
        logger.info("=" * 60)
