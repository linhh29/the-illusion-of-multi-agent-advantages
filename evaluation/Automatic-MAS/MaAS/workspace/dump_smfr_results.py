#!/usr/bin/env python3
"""Preview MaAS smfr eval JSON: first N lines or parsed per-level summary."""
...
def print_summary(path: Path) -> None:
    ...
    meta_keys = ("method", "smfr_eval", "task_type", "max_concurrent", "levels", "timestamp")
    print("=== meta ===")
    for k in meta_keys:
        if k in data:
            print(f"  {k}: {data[k]!r}")
    ...
    for key in sorted(per.keys(), key=lambda x: (len(x), x)):
        ...
        if isinstance(sa, dict):
            ...
            if any(x is not None for x in (df, cf, ef)):
                print(
                    f"    smfr_aggregate: direct_full={df} code_full={cf} "
...
def main() -> None:
    ...
    p.add_argument(
        "path",
        nargs="?",
        default="results_cot_smfr_run3.json",
        help="Path to results JSON (default: results_cot_smfr_run2.json in cwd)",
    )
