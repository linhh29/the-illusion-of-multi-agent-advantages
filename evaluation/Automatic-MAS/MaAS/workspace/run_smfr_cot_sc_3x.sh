#!/bin/bash
# Smfr CoT-SC evaluation. Levels 2–6, three runs.
# Up to 30 concurrent async SMFR tasks per run (asyncio.Semaphore in smfr_standalone_eval).
# Lower than CoT's 50 because each task fires num_samples LLM calls internally.
#
# Smoke test (one problem per level):   SMFR_LIMIT=1 bash workspace/run_smfr_cot_sc_3x.sh
...
for i in 1 2; do
    ...
    python -m workspace.standalone_cot_eval \
        ...
        --max_concurrent 50 \
        --max_tokens 32768 \
        --output_path "workspace/results_cot_sc_smfr_${MODEL}_run${i+1}.json"
    run_ec=$?
