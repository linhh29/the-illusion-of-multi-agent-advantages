# -*- coding: utf-8 -*-
"""
Multi-level SMFR standalone evaluation (AFlow-aligned scoring).
...
from maas.ext.maas.benchmark.smfr import (
    SMFR_AFLOW_INSTRUCTION,
    extract_smfr_reference_answer,
    parse_smfr_model_output,
    smfr_evaluate_direct_answer,
)
from maas.logs import logger

# Avoid interleaving when many SMFR requests print full inputs concurrently.
_smfr_llm_input_print_lock = threading.Lock()
...
def _merged_buckets_from_jsonl(merged_path: Path) -> Dict[int, List[Dict[str, Any]]]:
    """AFlow ``smfr_test.jsonl``：按 ``source_split``（难度 2–6）分桶。"""
...
def _resolve_smfr_merged_jsonl(explicit: Optional[str]) -> Optional[Path]:
    """合并版 jsonl：优先参数 / 环境变量，其次 ``mas_eval/AFlow/data/datasets/smfr_test.jsonl``。"""
    if explicit:
    ...
    env = os.environ.get("SMFR_MERGED_JSONL")
    if env:
        pe = Path(env).expanduser()
        if pe.is_file():
            return pe

    cand = _maas_repo_root().parent / "AFlow" / "data" / "datasets" / "smfr_test.jsonl"
    if cand.is_file():
        return cand
    return None
...
def _evaluate_code_output(
    ...
    """Same logic as ``SmfrBenchmark._evaluate_code_output``."""
...
def _smfr_aggregate_from_rows(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    total = len(rows)
    missing = sum(1 for r in rows if "smfr_eval_metrics" not in r)
    direct_full = sum(
        1 for r in rows if r.get("smfr_eval_metrics", {}).get("direct_full")
    )
    code_full = sum(1 for r in rows if r.get("smfr_eval_metrics", {}).get("code_full"))
    code_failed = sum(
        1 for r in rows if r.get("smfr_eval_metrics", {}).get("code_exec_failed")
    )
    code_ok_no_match = sum(
        1
        for r in rows
        if r.get("smfr_eval_metrics")
        and not r["smfr_eval_metrics"].get("code_full")
        and not r["smfr_eval_metrics"].get("code_exec_failed")
    )
    both = sum(
        1
        for r in rows
        if r.get("smfr_eval_metrics", {}).get("direct_full")
        and r.get("smfr_eval_metrics", {}).get("code_full")
    )
    denom = total - missing if total else 0
    return {
        "total_samples": total,
        "samples_missing_smfr_eval_metrics": missing,
        ...
        "scoring_note": "score==1.0 iff direct_full (AFlow SmfrsBenchmark); code_full is logged separately.",
        "interpretation": (
            ...
            "should equal total_samples when each row has smfr_eval_metrics (and typical non-empty reference answers): "
            "sandbox match, sandbox failure, or sandbox OK but structured answer did not match the reference."
        ),
    }


async def _call_smfr_llm(llm: Any, input_text: str) -> str:
...
def _smfr_row_llm_failed(
    ...
    logger.warning(f"SMFR sample failed, scored 0 (level={level}): {err_s}")
    logger.opt(exception=exc).debug("Full traceback for failed SMFR sample")
    ed: Dict[str, Any] = {
...
async def _evaluate_one(
    ...
    input_text = full_problem + SMFR_AFLOW_INSTRUCTION
    ref = extract_smfr_reference_answer(problem)
    ...
    with _smfr_llm_input_print_lock:
        print(
            f"[SMFR LLM] level={level} method={method_label} input_chars={len(input_text)} ref={ref}",
            flush=True,
        )

    try:
        output = await _call_smfr_llm(llm, input_text)
        ...
        raw_str, model_answer, code_str = parse_smfr_model_output(output)
        direct_full, direct_partial = smfr_evaluate_direct_answer(model_answer, ref)
        ...
        return _smfr_row_llm_failed(full_problem, ref, level, method_label, exc)
...
async def _evaluate_one_cot_sc(
    ...
    """Multiple SMFR calls; self-consistency by majority on ``parsed_model_answer``."""
    full_problem = problem.get("problem", "") or ""
    ref = extract_smfr_reference_answer(problem)
    ...
    direct_full, _ = smfr_evaluate_direct_answer(winner, ref)
    last = partial[-1]
    score = 1.0 if direct_full else 0.0
    ...
    return {
        ...
        "smfr_eval_metrics": last["smfr_eval_metrics"],
        "level": level,
    }


async def run_smfr_levels_async(
    *,
    smfr_levels: List[int],
    smfr_data_dir: str,
    ...
    smfr_merged_jsonl: Optional[str] = None,
    max_tokens: Optional[int] = None,
) -> Dict[str, Any]:
    os.environ["MAAS_CONFIG_PATH"] = config_path

    # Relative paths must not depend on shell CWD: always under MaAS repo root (same as smfr_data_dir).
    out_file: Optional[Path] = None
    if output_path:
        out_file = _resolve_under_maas(output_path)
        logger.info(f"SMFR results will be written to (resolved): {out_file}")
    ...
    base = _resolve_under_maas(smfr_data_dir)
    logger.info(f"SMFR data directory (resolved): {base}")
    all_results: List[Dict[str, Any]] = []
    per_level: Dict[str, Any] = {}
    sem = asyncio.Semaphore(max_concurrent)

    for level in smfr_levels:
        data_file = base / f"smfr_level_{level}.jsonl"
        ...
        async def run_one(p: Dict[str, Any]) -> Dict[str, Any]:
            ...
            done_count[0] += 1
            metrics = row.get("smfr_eval_metrics") or {}
            code_score = 1.0 if metrics.get("code_full") else 0.0
            print(
                f"[SMFR done] level={level} {done_count[0]}/{n_problems} "
        ...
        per_level[f"level_{level}"] = {
            ...
            "smfr_aggregate": _smfr_aggregate_from_rows(level_rows),
        }
    ...
    out: Dict[str, Any] = {
        "method": method,
        "smfr_eval": "aflow_smfr_benchmark",
        "task_type": "smfr",
        "max_concurrent": max_concurrent,
        "levels": smfr_levels,
        ...
        "overall": {
            ...
            "smfr_aggregate": _smfr_aggregate_from_rows(all_results),
    ...
    if out_file is not None:
        ...
        logger.info(f"Smfr results saved to {out_file}")

    return out


def run_smfr_levels(**kwargs: Any) -> Dict[str, Any]:
    return asyncio.run(run_smfr_levels_async(**kwargs))
