...
def main():
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None,
        help="Path to dataset (.jsonl file); not required when --smfr_levels is set",
    )
    parser.add_argument(
        "--output_path", 
        type=str, 
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Cap how many problems to run. With --smfr_levels: first N lines per level "
            "from each smfr_level_{n}.jsonl (same as limit_per_level). Example: --limit 1 for a smoke test."
        ),
    )
    parser.add_argument(
        "--instruction", 
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="If set, run SMFR multi-level eval (AFlow-aligned). Example: --smfr_levels 2 3 4 5 6",
    )
    parser.add_argument(
        "--smfr_data_dir",
        type=str,
        default="maas/ext/maas/data",
        help="Directory with smfr_level_{n}.jsonl; relative to MaAS repo root (…/MaAS/maas/ext/maas/data)",
    )
    parser.add_argument(
        "--max_concurrent",
        type=int,
        default=30,
        help="Max concurrent SMFR tasks (used with --smfr_levels)",
    )
    parser.add_argument(
        "--max_tokens",
        type=int,
    ...
    if args.smfr_levels is not None and len(args.smfr_levels) > 0:
        from workspace.smfr_standalone_eval import run_smfr_levels

        if args.output_path is None:
            args.output_path = (
                f"results_{args.method}_smfr_{Path(args.smfr_data_dir).name}.json"
            )

        os.environ["MAAS_CONFIG_PATH"] = args.config
        run_smfr_levels(
            smfr_levels=list(args.smfr_levels),
            smfr_data_dir=args.smfr_data_dir,
    ...
    if not args.data_path:
        raise SystemExit("error: --data_path is required unless --smfr_levels is provided")
