...
import glob
from utils import *
from prompt_lib import SYSTEM_PROMPT_BCP, construct_message, ROLE_MAP
...
async def generate_cot_response(question, model):
    ...
    user_message = {
        "role": "user",
        "content": f"Here is the question:\n{question}\n\nPlease solve this problem step by step. The answer may be a person's name, a place name, a number, or any other type of answer. At the end of your response, you must put your final answer in the form \\boxed{{your final answer}}. Make sure to include only the answer itself inside the boxed format, without any additional explanation."
    }
    ...
    answer = extract_math_answer(reply)
...
async def process_single_question(idx, que, ans, model, ensemble_size, dir_name, exp_name):
    ...
    try:
        ...
        for i in range(ensemble_size):
            ...
            # Debug: print if answer extraction failed
            if answer is None:
                print(f"Warning: Failed to extract answer from completion {i+1}. Reply preview: {reply[:200]}...")
        
        # Filter out None answers for voting, but keep them in the list for debugging
        valid_answers = [a for a in answers if a is not None]
        
        # Use voting to get the final answer
        if len(valid_answers) > 0:
            final_answer, vote_count = most_frequent(valid_answers, lambda x, y: x == y)
        else:
            # If all answers are None, try to extract from the last completion as fallback
            print(f"Warning: All {ensemble_size} answers were None. Attempting fallback extraction...")
            if len(completions) > 0:
                final_answer = extract_math_answer(completions[-1])
                if final_answer is None:
                    # Last resort: try to extract from any completion
                    for completion in completions[::-1]:
                        final_answer = extract_math_answer(completion)
        ...
        if final_answer is not None:
            ...
            # Only convert to uppercase if it's a single letter (for compatibility with old code)
            # For names, places, etc., keep original case
        ...
        # Pass empty string if final_answer is None to avoid issues
        answer_for_grading = final_answer if final_answer is not None else ""
        acc = grade_bcp_answer(que, answer_for_grading, ans)
...
async def main():
    ...
    # Ensemble size: optional argv[7] overrides; else use default ensemble size of 5
    ...
    qa_pairs = get_bcp_qa_pairs(QUERY_JSONL)
