...
import glob
from utils import *
from prompt_lib import SYSTEM_PROMPT_HLEMATH, construct_message, ROLE_MAP_MATH
...
async def generate_cot_response(question, model):
    ...
    user_message = {
        "role": "user",
        "content": f"Here is the question:\n{question}\n\nPlease solve this problem step by step. Put your final answer in \\boxed{{answer}} format at the end of your response."
    }
    
    contexts = []
    contexts.append(user_message)
    
    # Generate answer using async version for concurrent execution
    reply, prompt_tokens, completion_tokens = await generate_answer_async(contexts, model)
...
async def process_single_question(idx, que, ans, model, ensemble_size, dir_name, exp_name):
    ...
    try:
        # Generate multiple CoT responses concurrently
        print(f"Generating CoT-SC responses 1-{ensemble_size} of {ensemble_size} (concurrent)...")
        
        # Create all tasks for concurrent execution
        tasks = [generate_cot_response(que, model) for _ in range(ensemble_size)]
        results = await asyncio.gather(*tasks)
        
        # Extract results
        completions = [result[0] for result in results]
        answers = [result[1] for result in results]
        total_prompt_tokens = sum(result[2] for result in results)
        total_completion_tokens = sum(result[3] for result in results)
        
        # Use voting to get the final answer
        final_answer, vote_count = most_frequent(answers, lambda x, y: is_equiv(x, y))
        
        # Evaluate the answer
        acc = is_equiv(final_answer, ans) if final_answer else False
...
async def main():
    ...
    qa_pairs = get_hlemath_qa_pairs(QUERY_JSONL)
