# -*- coding: utf-8 -*-
"""
SMFR dataset utils: data loading and evaluation logic.
Evaluation logic is aligned with AFlow benchmarks/smfr.py.
"""
import ast
import json
import os
...
def get_smfr_qa_pairs(jsonl_path: str, require_code: bool = True) -> List[Tuple[str, Dict]]:
    """
    Load SMFR problems from JSONL. Each line: {"problem": "...", "answer": ...}.
...
# ---------- Parse model output (same as AFlow SmfrsBenchmark._parse_model_output) ----------
...
# ---------- Code evaluation (same as AFlow SmfrsBenchmark._evaluate_code_output) ----------
...
def most_frequent(clist, cmp_func):
    """Return most frequent element and its count (for CoT-SC consensus)."""
    if not clist:
...
# ---------- OpenAI client and generate_answer ----------
_client = None
...
def _debug_print_chat_completion_kwargs(common_kwargs: Dict[str, Any], max_content: int = 4000) -> None:
    ...
    try:
        ...
        print("[SMFR utils] chat.completions.create kwargs (preview):\n", preview, sep="", flush=True)
    except Exception as e:
        print("[SMFR utils] json.dumps(trimmed kwargs) failed:", e, flush=True)
    try:
        raw = json.dumps(common_kwargs, ensure_ascii=False)
    except Exception as e:
        print("[SMFR utils] strict json.dumps(full kwargs) failed:", e, flush=True)
    else:
        print(
            "[SMFR utils] strict json.dumps(full kwargs) OK, byte length:",
...
def generate_answer(answer_context, model):
    """
    Call OpenAI chat completion with JSON schema response_format for SMFR.
...
# ---------- Parser: return raw reply for smfr ----------
def parse_smfr_raw(reply):
