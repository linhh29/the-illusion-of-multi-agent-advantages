...
async def process_single_question(idx, instance_id, text, expected_patch, model, dir_name, exp_name, 
    ...
    try:
        ...
        if enable_evaluation and judge_path and file_path:
            ...
            solution_name = f"cot_{idx}"
            
            for attempt in range(max_retries):
                # Generate new CoT response if this is a retry (attempt > 0) or first attempt
                if attempt == 0 or (final_patch is None):
                    ...
                    completion = reply
                    final_patch = patch if patch else ""
                ...
                try:
                    ...
                    eval_result = await run_swebench_evaluation(
                        judge_path, instance_id, final_patch, "cot", solution_name, 
                    ...
                    if error_instances == 1:
                        if attempt < max_retries - 1:
                            print(f"Instance {idx+1} ({instance_id}): Docker error detected, regenerating CoT response, retrying...")
            ...
            # No evaluation enabled, just generate CoT response once
            reply, patch, prompt_tokens, completion_tokens = await generate_cot_response(text, model)
            completion = reply
            final_patch = patch if patch else ""
            total_prompt_tokens = prompt_tokens
...
async def main():
    ...
    print(f"Processing {len(qa_pairs)} instances with CoT (max {MAX_CONCURRENT} concurrent)")
    ...
    async def process_with_semaphore(idx, instance_id, text, expected_patch):
        async with semaphore:
            result = await process_single_question(
                idx, instance_id, text, expected_patch, MODEL, DIR_NAME, EXP_NAME,
    ...
    write_jsonl(DIR_NAME+'/'+EXP_NAME+'_cot.jsonl', results_jsonl)
